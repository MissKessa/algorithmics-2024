...
public class ImageBnB {
	
	private float[][] img;
	private int width, height;
	
	//constructor to open a picture
	public ImageBnB(String img_path) {
		try 
		{
	...
	public ImageBnB(int width, int height) {
		this.width = width;
		this.height = height;
	...
	public void addSignal(ImageBnB img) {
		for (int i = 0; i < width; i++)
	        for (int j = 0; j < height; j++)
	...
	//inverts pixel values
	public ImageBnB getInvertedImage() {
		invertSignal();
		return this;
	}
	...
	public float zncc(ImageBnB img) {
		float cc = 0;
		float std_i = 0;
	...
	public ImageBnB copy() {
		ImageBnB hold_img = new ImageBnB(this.width, this.height);
