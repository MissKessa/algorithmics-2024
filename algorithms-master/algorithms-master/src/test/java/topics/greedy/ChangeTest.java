...
public class ChangeTest {
	...
	public void testOptimal() {
		...
		for (int i = 0; i < solution.length; i++) {
			...
			log.trace("\tThe number of coins of value " + coins[i] + " is " + solution[i]); // sometimes it gives the
																							// best solution
		}
	}
	...
	public void testNotOptimal() {
		...
		for (int i = 0; i < solution.length; i++) {
			assertEquals(expectedSolution[i], solution[i]);
			log.trace("\tThe number of coins of value " + coins[i] + " is " + solution[i]); // other times it doesn't
