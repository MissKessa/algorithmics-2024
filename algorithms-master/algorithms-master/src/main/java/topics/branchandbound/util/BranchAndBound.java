...
public abstract class BranchAndBound {
	...
	public void branchAndBound(Node rootNode) {
		...
		while (!ds.empty() && ds.estimateBest() < pruneLimit) { // there are still nodes with a better heuristic
			...
			for (Node child : children)
				if (child.isSolution()) {
					...
					if (cost < pruneLimit) { // it's a better solution
						pruneLimit = cost;
						bestNode = child;
					}
				} else if (child.getHeuristicValue() < pruneLimit) { // if it's not a solution, but it can be one
					ds.insert(child);
				}
		} // while
	}
