...
public class EightPuzzle extends BranchAndBound {
	 ...
	 * 
	 * @param heuristicType Type of the heuristic used to solve the problem -
	 *                      Manhattan or WrongPlace
...
class Puzzle extends Node {
	...
	public Puzzle(int[] board, HeuristicType heuristicType, int depth, UUID parentID) { // Constructor for children
		this.board = board;
		this.heuristicType = heuristicType;
		this.depth = depth;
	...
	/*
	 * Returns a copy of the board but in this case, moving the empty cell upwards
	 */
	private int[] up() {
		int[] temp = board.clone();
		int emptyCell = 0;
		for (int i = 0; i < temp.length; i++) {
			if (temp[i] == 9) // Value of empty
				emptyCell = i; // Store position
		}
		// Invalid option check
		if (emptyCell < 3) // It is not possible to move the piece in the indicated direction (empty cell
							// in 0, 1 or 2)
			return temp;
	...
	/*
	 * Returns a copy of the board but in this case, moving the empty cell downwards
	 */
	private int[] down() {
		int[] temp = board.clone();
		int emptyCell = 0;
		...
		// Invalid option check
		if (emptyCell > 5) // It is not possible to move the piece in the indicated direction (empty cell
							// in 6, 7 or 8)
			return temp;
		temp[emptyCell] = temp[emptyCell + 3]; // 3 is the length of the board
		temp[emptyCell + 3] = 9;
		return temp;
	}

	/*
	 * Returns a copy of the board but in this case, moving the empty cell to the
	 * left
	 */
	private int[] left() {
		int[] temp = board.clone();
		...
		// Invalid option check
		if (emptyCell % 3 == 0) // It is not possible to move the piece in the indicated direction (empty cell
								// in 0,3 or 6)
			return temp;
	...
	private int[] right() {
		...
		// Invalid option check
		if (emptyCell % 3 == 2) // It is not possible to move the piece in the indicated direction (empty cell
								// in 2, 5 or 8)
			return temp;
		temp[emptyCell] = temp[emptyCell + 1];
		temp[emptyCell + 1] = 9;
	...
	private boolean prune() { // PRUNING METHOD
		...
		int sum = 0;
		for (int i = 1; i <= 9; i++) {
			sum += smaller(i); // Sum smaller values
		}
		if ((position(9) % 2) == 0) // Empty cell is in even position
			x = 1;
	...
	/*
	 * It is the number of pieces such that j<i and position(j) > position(i)
	 */
	...
	public void calculateHeuristicValue() {
		if (prune())
			heuristicValue = Integer.MAX_VALUE; // Not pick it for the heap
		else {
	...
	public boolean isSolution() {
		return (getHeuristicValue() == 0) ? true : false; // No wrong tiles
	}
