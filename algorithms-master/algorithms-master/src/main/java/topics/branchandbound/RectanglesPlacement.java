...
class Game extends Node {
	...
	// Generates a new board and pieces (to place them in the board in the best way
	// possible) (ROOT NODE)
	public Game(int n, List<Piece> pieces) {
	...
	private boolean insertNewPiece(int x, int y, PieceOrientation orientation, int[][] newBoard, Piece piece) {
		...
		if (orientation == PieceOrientation.Horizontal) { // The default orientation
			limitX = piece.x;
			limitY = piece.y;
		} else { // If the orientation is vertical we need to interchange the coordinates of the
					// piece
			limitX = piece.y;
			limitY = piece.x;
		}
