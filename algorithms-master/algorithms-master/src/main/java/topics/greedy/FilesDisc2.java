...
/**
 * GREEDY ALGORITHM PROBLEM: MINIMIZE THE FREE SPACE ON A DISK WITH FILES It has
 * NOT an optimal solution in some cases. The idea is: First the biggest files
 * while the disc has enough space to copy them
 * 
 * @author viceg
 */
public class FilesDisc2 {
	 ...
	 */
	public int calculate() { // O(n)
		Quicksort quicksort = new Quicksort();
		quicksort.sort(files);
		int i = files.length - 1; // we start at the end as files are sorted. For example: [50, 100, 450, 5000]
		int counter = 0;
		while (i >= 0) { // until I finish
			if ((counter + files[i]) <= discCapacity) {
				counter += files[i];
			}
			i--;
		}
		return counter; // the counter is the space occupied
	}
