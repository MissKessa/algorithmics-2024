...
/**
 * GREEDY ALGORITHM PROBLEM: THE HORSE JUMPING PROBLEM (Knight's tour) It has
 * not an optimal solution in some cases
 * 
 * @author viceg
 */
public class ChessHorse {
	...
	public ChessHorse(int n) {
		...
		// Set all board to -1 (not visited yet)
		for (int i = 0; i < n; i++)
			for (int j = 0; j < n; j++)
				board[i][j] = -1; // The number indicates the order in which the horse goes through that cell (-1
									// if nothing)
	}
	...
	public boolean horse(int[] pos) {
		...
		for (int i = 0; i < n * n; i++) { // i indicates the number of movement. We need to cover the entire board
			...
			if (!newMovement(pos, newPos)) // If we don't have a movement => we fail
				if (i != n * n - 1)
					return false; // Unless that is the last element of all
	...
	private boolean newMovement(int[] pos, int[] newPos) { // heuristic
		...
		for (int i = 1; i <= 8; i++) { // Different types of jumps. We should try all to study the position from which
										// we have less possible jumps
			if (jump(pos, i, newPos)) { // If we can do that movement, we keep the new possible position in the newPos
										// variable
				hasMovement = true; // We have at least one possible movement
				int result = jumpCounter(newPos); // We should choose the one from which we have less accessible jumps
													// (our heuristic)
				if (result < minAccessible) {
					minAccessible = result;
					tempPos[0] = newPos[0]; // We need tempPos to keep the best result because the value of newPos is
	...
	/*
	 * Returns the number of cells to which the horse can jump from a given position
	 */
	private int jumpCounter(int[] pos) {
		int count = 0;
		for (int i = 1; i <= 8; i++) { // Different types of jumps
			if (jump(pos, i, new int[2]))
				count++;
		}
		return count;
	}
	...
	private boolean jump(int[] pos, int jumpType, int[] newPos) {
		...
		switch (jumpType) { // here we have all possible moves
		case 1:
			newPos[0] = pos[0] + 2;
			newPos[1] = pos[1] + 1;
			break;
		...
		return ((newPos[0] >= 0) && (newPos[0] < n) // The new position for X is within the board
				...
				&& (board[newPos[0]][newPos[1]] == -1)); // The new position is empty (check we haven't gone yet)
	}
