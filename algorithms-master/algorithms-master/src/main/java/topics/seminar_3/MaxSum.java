...
/**
 * DIVIDE AND CONQUER PROBLEM: MAXIMUM SUMMATION OF ALL THE SUBSEQUENCES OF n
 * ELEMENTS
 * 
 * @author viceg
 */
public class MaxSum {
	 ...
	 */
	public int maxSum1(int[] v) { // MUCH WORSE
		int n = v.length;
	 ...
	 * @return The maximum sum of the subsequences
	 */
	public int maxSum2(int[] v) { // GOOD
		int n = v.length;
		int max = 0;
		for (int i = 0; i < n; i++) {
			int sum = 0;
			for (int j = i; j < n; j++) { // we always sum the numbers between i and n
				sum = sum + v[j];
				if (sum > max)
					max = sum;
			}
	...
	/**
	 * This algorithms is recursive. It is DandC by division with a=2,b=2,k=1 -
	 * O(nlogn)
	 * 
	 * @param v Vector to calculate the values
	 * @return The maximum sum of the subsequences
	 */
	public int maxSum3(int[] v) { // BEST
	...
	private int maxSumByDivision(int left, int right, int[] v) {
		if (left == right)
			...
			// calculates the maximum of the subsequence that passes from one-half to the
			// other half
			int sum1 = 0;
			int maxSum1 = 0;
