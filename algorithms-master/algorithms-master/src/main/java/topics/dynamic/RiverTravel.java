...
/**
 * DYNAMIC PROGRAMMING PROBLEM: CHEAPER TRAVEL ON THE RIVER It consists on
 * finding the minimum cost for each pair of points
 * 
 * @author viceg
 */
public class RiverTravel {
	...
	public void riverTravel(int[][] t, int[][] c) {
		...
		for (int i = n - 2; i >= 0; i--) // Be careful, it is a descendant loop
			for (int j = i + 1; j < n; j++) { // start where we can go
				int min = Integer.MAX_VALUE;
				for (int k = i + 1; k <= j; k++) { // start where we can go
					int valueK = 0;
					if (k == j)
						valueK = t[i][j];
					else
