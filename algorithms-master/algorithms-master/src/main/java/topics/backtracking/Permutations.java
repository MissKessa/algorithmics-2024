...
/**
 * BACKTRACKING PROBLEM: PERMUTATIONS OF N ELEMENTS This program generates
 * permutations of the integer elements that are in the vector v
 * 
 * @author viceg
 */
public class Permutations {
	...
	private boolean[] mark; // To mark elements that are already used in any of the solutions (to avoid
	...
	public Permutations(int n) {
		...
		for (int i = 0; i < n; i++) // Generates a vector of size n -> {0, 1, 2, 3 .. n}
			v[i] = i;
	...
	private void backtracking(int level) {
		if (level == n) { // There is already a solution or complete permutation
			...
			counter++; // Increase counter of the solution
		} else {
			for (int j = 0; j < n; j++) // Tries new permutations (new sequences of elements)
				if (!mark[j]) { // If the element is not marked in this permutation
					// I DO THINGS
					sol[level] = v[j]; // ...Uses it -> Level at the number is used
					mark[j] = true; // ...And marks it

					// Go to the next level:
					backtracking(level + 1); // When the backtracking ends (after printing its permutation), unmarks the
												// previously marked numbers
