...
/**
 * BACKTRACKING PROBLEM: THE PROBLEM OF THE N QUEENS This program calculates one
 * way of placing n queens on a chessboard of side n
 * 
 * @author viceg
 */
public class ChessQueensOne {
	 ...
	 */
	public void backtracking(int j) { // j is the column
		if (j == n) { // We have already placed the n queens
			...
			for (int i = 0; i < n; i++)
				if (!a[i] && !b[i + j] && !c[i - j + n - 1] && !found) { // !a[i] -> row not occupied; !b[i + j] ->
																			// right diagonal not occupied; !c[i - j + n
																			// - 1] -> left diagonal not occupied;
																			// !found -> a solution hasn't been found
					// Set queen:
					...
					backtracking(j + 1); // Go to the next column
